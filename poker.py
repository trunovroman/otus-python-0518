...
def internal_best_hand(hand_list):
    """Универсальная функция, которая используется для поиска лучших рук.
    На вход подается список рук по 7 карт, на выходе выбирается лучшая рука из 5 карт.
    Используется и в best_hand и в best_wild_hand"""
